...
:root{
  --navy:#0B1F3A;--gold:#C9A84C;--gold-lt:#e0c068;
...
body{background:var(--navy);font-family:'Inter',sans-serif;-webkit-tap-highlight-color:transparent;}
#root{min-height:100vh;}
.app{min-height:100vh;background:linear-gradient(160deg,#070f1e 0%,#0B1F3A 45%,#0d2040 100%);color:var(--text);}
...
.mrow{background:rgba(255,255,255,0.025);border:1px solid var(--bd);border-radius:12px;padding:12px 14px;display:flex;align-items:center;gap:10px;transition:border-color .15s;}
...
.nav-bar{position:fixed;bottom:0;left:0;right:0;background:rgba(7,16,32,.97);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px);border-top:1px solid var(--gold-bd);display:flex;justify-content:space-around;padding:8px 0;padding-bottom:max(8px,env(safe-area-inset-bottom));z-index:300;}
...
.toast{position:fixed;bottom:88px;left:50%;transform:translateX(-50%);background:#0d1e36;border:1px solid var(--gold-bd);color:var(--gold);padding:11px 22px;border-radius:12px;font-weight:600;font-size:14px;z-index:999;white-space:nowrap;animation:fadeUp .25s ease;box-shadow:0 8px 30px rgba(0,0,0,.5);}
...
.topbar{background:linear-gradient(90deg,#060e1c,#0B1F3A);border-bottom:1px solid var(--gold-bd);padding:10px 18px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:200;}
